...
def make_section_summary(section: str, sentence: str | None) -> tuple[str, str]:
    ...
    return (
        SECTION_SUMMARY_ZH_PREFIX[section] + sentence,
        SECTION_SUMMARY_EN_PREFIX[section] + sentence,
    )
...
def extract_key_signals(evidence: list[dict[str, Any]], limit: int = 6) -> list[str]:
    ...
    signals = []
    for value in _re.findall(r'\d+(?:\.\d+)?%?|\d{4}', text):
...
def make_deep_summary_zh(section: str, selected: dict[str, Any] | None, evidence: list[dict[str, Any]], topic: str) -> str:
    ...
    page_text = '、'.join(f'第{p}页' for p in pages) if pages else '当前解析文本'
    signals = extract_key_signals(evidence)
    signal_text = ('含数字线索：' + '、'.join(signals) + '。') if signals else ''
    details = SECTION_DETAIL_ZH[section]
    if not (selected and selected.get('summary')):
        return '当前解析文本未检测到本栏目的相关线索，建议人工结合原文补写。'
    count_text = f'{ev_count} 条相关片段'
    return ('已从' + page_text + '检测到本栏目相关线索（' + count_text + '）。'
            + details['role'] + signal_text
            + '具体证据见下方「查看原文证据」，需结合原文页码逐句核对。')
